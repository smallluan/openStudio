...
export default function ChatLabOrchestrationPlanStrip({
  ...
  return (
    ...
    >
      ...
      {awaiting ? (
        <p className="chat-lab__orch-plan-strip-awaiting-hint" role="status">
          {t("orchestration.planProgress.awaitingHint")}
        </p>
      ) : null}
      {plan.summary ? <p className="chat-lab__orch-plan-strip-summary">{plan.summary}</p> : null}
      <ol className="chat-lab__orch-plan-strip-phases">
        {plan.tasks.map((task) => {
          const done = task.status === "done";
          const active = task.id === run.activeTaskId || task.status === "in_progress";
          ...
          return (
            ...
            >
              ...
              {owner || roleLabel ? (
                <span className="chat-lab__orch-plan-strip-phase-meta">
