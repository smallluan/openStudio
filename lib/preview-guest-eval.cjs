 ...
 * Used by native tool `browser_eval` and tab page-script presets.
 */
...
async function handleSidebarEval(args = {}) {
  ...
  if (isDebuggerPaused()) {
    return {
      ...
      message:
        "Page is paused on a breakpoint. Use browser_debugger op=evaluate in the paused frame, or op=resume first.",
    };
  }
