 ...
 */
function ensureDevGatewayAuthToken(stateDir = resolveDevOpenClawStateDir()) {
  if (!isStudioManagedOpenClawStateDir(stateDir)) {
    return { ok: false, reason: "not_studio_managed_profile" };
  }
...
function mapStudioProfileToOpenClaw(profile) {
  ...
  if (provider === "anthropic-compatible") {
    ...
    return {
      ...
      ...(isValidHttpBaseUrl(customBaseUrl) ?
        {
          gatewayCatalog: {
...
function patchOpenClawGatewayStudioBinding(stateDir, agentId, modelRef, providerPluginId, studioCfg, gatewayCatalog) {
  ...
  let changed = false;
  const devProfile = isStudioManagedOpenClawStateDir(stateDir);
  ...
  // Tool Search: hide full schemas behind tool_search / tool_describe / tool_call.
  // Prefer structured `tools` mode — DeepSeek handles JSON tool calls far better than
  // the experimental `tool_search_code` JS bridge. Studio prompts teach search→call.
  cfg.tools ??= {};
  const tools = /** @type {Record<string, unknown>} */ (cfg.tools);
  const wantToolSearch = {
    mode: "tools",
    searchDefaultLimit: 8,
    maxSearchLimit: 20,
  };
  const prevTs = tools.toolSearch;
  const sameToolSearch =
    prevTs &&
    typeof prevTs === "object" &&
    !Array.isArray(prevTs) &&
    /** @type {{ mode?: unknown }} */ (prevTs).mode === wantToolSearch.mode &&
    /** @type {{ searchDefaultLimit?: unknown }} */ (prevTs).searchDefaultLimit ===
      wantToolSearch.searchDefaultLimit &&
    /** @type {{ maxSearchLimit?: unknown }} */ (prevTs).maxSearchLimit === wantToolSearch.maxSearchLimit;
  if (!sameToolSearch) {
    tools.toolSearch = wantToolSearch;
    changed = true;
  }
...
function patchAllEnabledProviderCatalogs(studioCfg, stateDir) {
  ...
  const activeId =
    typeof studioCfg.activeModelProfileId === "string" ? studioCfg.activeModelProfileId.trim() : "";
  const orderedIds = [
    ...enabledIds.filter((id) => id !== activeId),
    ...(activeId && enabledIds.includes(activeId) ? [activeId] : []),
  ];
