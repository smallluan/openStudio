 ...
 * `plugins.allow` in openclaw.json. We persist them ourselves so the user
 * gets the full dev tool surface (browser/phone-control/file-transfer/...) for
 * letting the model actually drive the computer — Open Studio is *not* a
 * chat-only client.
 *
 * Tradeoff: the more plugins are listed here, the longer `core-plugin-tools`
 * takes on the **first** chat after a fresh process start, especially on a
 * cold Windows boot where the file cache is empty. We've measured up to
 * ~85s for `core-plugin-tools` with this full set on a freshly rebooted
 * machine; Open Studio mitigates this during the desktop bootstrap screen by
 * calling `tools.catalog` (registry), `sessions.create` (ensure the session key
 * exists), then `tools.effective` (builds the same `createOpenClawCodingTools`
 * surface as the first chat) — **no user chat turn**, but local gateway work
 * can still resemble a real send.
 */
