/**
 * Loopback HTTP bridge: OpenClaw `sidebar_action` tool → Electron renderer automation.
 *
 * Gateway process POSTs here; main process forwards to the focused/main BrowserWindow
 * via IPC and returns the JSON tool result (execution + fresh DOM inventory).
 */
  ...
  server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/health")) {
        sendJson(req, res, 200, { ok: true, service: "open-studio-sidebar-action" });
        return;
      }

      if (req.method !== "POST" || (url.pathname !== "/v1/sidebar_action" && url.pathname !== "/sidebar_action")) {
      ...
      const result = await dispatchToRenderer(body);
