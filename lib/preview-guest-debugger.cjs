 ...
 * Used by native tool `browser_debugger`.
 */
...
/**
 * Compact pause snapshot for tools / browser_action early-return (no extra CDP).
 */
...
async function ensureSession(wc) {
  ...
  return {
    ...
    guestId: wc.id,
    hint: "Like DevTools global search: break_on_text searches all loaded scripts and sets a breakpoint on each match; also watches future dynamic chunks. When a breakpoint hits: host pause UI + browser_action returns debuggerPaused:true — then inspect / resume.",
  };
}
...
async function breakOnText(wc, args = {}) {
  ...
  let hint =
    okCount > 0
      ? `Set ${okCount} breakpoint(s). Reproduce with browser_action — if hit, it returns debuggerPaused:true (+ inspect). After inspecting, ALWAYS op=resume before retrying or more clicks (yellow bar / page stays frozen until resume).`
      : watchDynamic
...
async function breakOnLocation(wc, args = {}) {
  ...
  return {
    ...bp,
    hint: bp.ok ? "Reproduce with browser_action, then call waitPaused." : bp.hint,
  };
}
...
function formatPauseResult(state) {
  ...
  return {
    ...
    callFrames,
    hint:
      "BREAKPOINT HIT — page JS is frozen (pause bar on preview). Next: op=inspect / evaluate, then ALWAYS op=resume before any further browser_action or retry. Leaving it paused blocks the user.",
  };
}
