...
async function ensureLocalGatewayRunning(getCfgFn, deps) {
  ...
  const cliResolved = resolveOpenClawCliPath();
  if (!cliResolved) {
  ...
  const childEnv = {
    ...
    NODE_PATH: buildBundledCliNodePath(),
  };
 ...
 * @returns {string}
 */
function buildBundledCliNodePath() {
  /** @type {string[]} */
  const segments = [];
...
function resolveBundledOpenClawPackageRootSync() {
  const log = getStudioLog();
