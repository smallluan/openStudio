...
const { createGatewaySubagentTracker } = require("./gateway-subagent-tracker.cjs");
 ...
 * (milliseconds; `0` disables the watchdog, otherwise min 30000). RPC completions
 * (`tools.*`, `sessions.*`) do **not** reset this timer — only WebSocket push
 * `event` frames do — so embedded runs that stay silent on the event channel need
 * a generous default (1 hour). */
function resolveStreamNoProgressTimeoutMs() {
  ...
  return Number.isFinite(n) && n >= 30_000 ? n : 3_600_000;
}
...
/**
 * Prefer the longer assistant body when chat payloads briefly regress or diverge.
 * @param {string} prev
 * @param {string} incoming
 */
function preferLongerChatText(prev, incoming) {
  const a = typeof prev === "string" ? prev : "";
  const b = typeof incoming === "string" ? incoming : "";
  if (!b.trim()) return a;
  if (!a.trim()) return b;
  if (b.startsWith(a) || a.startsWith(b)) return b.length >= a.length ? b : a;
...
function extractChatPayloadSnapshot(p) {
  ...
  const text = preferLongerChatText(fromMsg.text, fromRest.text);
...
/**
 * Estimate Studio-side outbound prompt parts (chars → ~tokens) for usage breakdown.
 * Gateway session memory + tool schemas are not included — see `estGatewayOverheadTokens`.
 *
 * @param {Array<{ role: string; content: string }>} messages
 * @param {unknown} [composerSkill]
 * @param {{ channel?: "internal" | "wechat"; contextEmbedMode?: "none" | "bootstrap" | "incremental" | "full"; threadSummaryPrefix?: string }} [opts]
 */
function measureOutboundMessageParts(messages, composerSkill, opts = {}) {
  if (!Array.isArray(messages)) {
    return {
      estSystemTokens: 0,
      estHistoryTokens: 0,
      estUserTokens: 0,
      estStudioInputTokens: 0,
      outboundChars: 0,
      priorTurnCount: 0,
      contextEmbedMode: opts.contextEmbedMode ?? "full",
    };
  }
  const channel = opts.channel === "wechat" ? "wechat" : "internal";
  const embedMode = opts.contextEmbedMode ?? "full";
  ...
  const systemBlock = systemParts.length
    ? `[System instructions]\n${systemParts.join("\n\n")}\n`
    : "";
  const wechatBlock = channel === "wechat" ? `${WECHAT_CHAT_SEND_PREFIX}\n\n` : "";

  let historyBlock = "";
  let userBlock = latest;
  if (prior.length === 0) {
    if (peeledLatest?.label?.startsWith("Agent ·")) {
      userBlock = `${currentHeading}\n${latest}`;
    }
  } else {
    const summaryPrefix =
      typeof opts.threadSummaryPrefix === "string" ? opts.threadSummaryPrefix.trim() : "";
  ...
  const full = buildChatSendMessage(messages, composerSkill, opts);
  const estSystemTokens = estimateTokensFromText(systemBlock + wechatBlock);
  const estHistoryTokens = estimateTokensFromText(historyBlock);
  const estUserTokens = estimateTokensFromText(userBlock);
  return {
    estSystemTokens,
    estHistoryTokens,
    estUserTokens,
    estStudioInputTokens: estimateTokensFromText(full),
    outboundChars: full.length,
...
async function dispatchOpenClawGatewayStreamWithResolved(resolved, messages, signal, onEvent, opts = {}) {
  ...
  let gatewayAcceptedAt = /** @type {number | null} */ (null);
  ...
  const emit = createChatStreamEmitter((evt) => {
    if (!firstDeltaAt && (evt.type === "text" || evt.type === "thinking")) {
      ...
      log.info("[chat.send.perf] first_delta", {
        ...
        agentTtftMs: gatewayAcceptedAt ? firstDeltaAt - gatewayAcceptedAt : null,
        type: evt.type,
      });
    }
    onEvent(evt);
  ...
  const subagentTracker = createGatewaySubagentTracker({
    parentRunId: runRef.id,
    // Parallel spawn + mid-turn yield barrier settles with the parent turn;
    // do not keep parent stream in a post-completion grace window.
    postCompletionGraceMs: 0,
  });
  subagentTracker.registerParentRunId(runRef.id);
  /** @type {ReturnType<typeof setTimeout> | null} */
  let subagentFinishTimer = null;
  /** True once parent `chat` event with state=final has been applied. */
  let chatFinalReceived = false;
  /**
   * Parent `lifecycle.end` can arrive before `chat.final`. Closing immediately
   * drops trailing prose mid-sentence (e.g. "需要我展开某条新闻的"). Wait briefly.
   */
  const CHAT_FINAL_GRACE_MS = 1500;
  /** @type {ReturnType<typeof setTimeout> | null} */
  let chatFinalGraceTimer = null;

  const clearSubagentFinishTimer = () => {
    if (!subagentFinishTimer) return;
    clearTimeout(subagentFinishTimer);
    subagentFinishTimer = null;
  };

  const clearChatFinalGraceTimer = () => {
    if (!chatFinalGraceTimer) return;
    clearTimeout(chatFinalGraceTimer);
    chatFinalGraceTimer = null;
  };

  const emitSubagentSnapshots = () => {
    for (const payload of subagentTracker.snapshotActivityPayloads()) {
      emit({
        type: "agent_activity",
        stream: "subagent",
        runId: runRef.id,
  ...
  const finishStreamNow = () => {
    clearSubagentFinishTimer();
    clearChatFinalGraceTimer();
    finishWaiter?.();
  };

  /**
   * Finish only after children settle, and prefer waiting for chat.final so the
   * last summary tokens are not lost when lifecycle.end races ahead.
   */
  const finishAfterChatFinalOrGrace = () => {
    if (finished) return;
    if (!subagentTracker.canFinishStream()) {
      scheduleSubagentFinishCheck();
      return;
    }
    ...
    if (chatFinalGraceTimer) return;
    chatFinalGraceTimer = setTimeout(() => {
      chatFinalGraceTimer = null;
      if (finished) return;
      if (subagentTracker.canFinishStream()) finishStreamNow();
    }, CHAT_FINAL_GRACE_MS);
    if (typeof chatFinalGraceTimer.unref === "function") chatFinalGraceTimer.unref();
  };

  const scheduleSubagentFinishCheck = () => {
    clearSubagentFinishTimer();
    if (!subagentTracker.shouldHoldStreamOpen()) {
      if (subagentTracker.canFinishStream()) finishAfterChatFinalOrGrace();
      return;
    }
    subagentFinishTimer = setTimeout(() => {
      subagentFinishTimer = null;
      if (subagentTracker.canFinishStream()) finishAfterChatFinalOrGrace();
      else scheduleSubagentFinishCheck();
    }, 50);
    if (typeof subagentFinishTimer.unref === "function") subagentFinishTimer.unref();
  };

  const tryFinishAfterParentFinal = () => {
    emitSubagentSnapshots();
    finishAfterChatFinalOrGrace();
  };
  ...
  function emitChatSnapshotDeltas(snap) {
    if (snap.thinking.length > lastThinking.length && snap.thinking.startsWith(lastThinking)) {
      ...
      } else if (snap.thinking && snap.thinking.length >= lastThinking.length) {
        lastThinking = snap.thinking;
        emit({
          type: "content_sync",
          content: lastText,
    ...
    if (snap.text.length > lastText.length && snap.text.startsWith(lastText)) {
      ...
      if (lastText.startsWith(snap.text)) {
        ...
        // Divergent rewrite: never replace a longer accumulated body with a
        // shorter snapshot (final payloads can arrive mid-URL / mid-sentence).
  ...
  const completion = new Promise((resolve, reject) => {
    finishWaiter = (reason) => {
      ...
      clearChatFinalGraceTimer();
      clearSubagentFinishTimer();
      // Lifecycle-driven finish can race ahead of chat.final — push the best
      // accumulated body so the renderer does not freeze on a mid-sentence slice.
      if (reason !== "error" && (lastText.trim() || lastThinking.trim())) {
        emit({ type: "content_sync", content: lastText, thinking: lastThinking });
      }
      if (typeof emit.flush === "function") emit.flush();
      finished = true;
      if (reason === "error") {
  ...
  function eventTargetsThisRun(p) {
    ...
    if (rid && subagentTracker.isKnownChildRun(rid)) return true;
    // Child sessions use a different sessionKey; claim pending spawn → live runId.
    const eventSessionKey = typeof p.sessionKey === "string" ? p.sessionKey.trim() : "";
    if (
      rid &&
      subagentTracker.tryClaimChildRun(rid, runRef.id, {
    ...
    if (rid && !subagentTracker.acceptsRunId(rid, runRef.id)) return false;
    return true;
  }
  ...
  const offEvent = client.onEvent((evt) => {
    if (evt.event === "agent") {
      ...
      const rid = typeof p.runId === "string" ? p.runId.trim() : "";
      if (rid && !subagentTracker.isKnownChildRun(rid)) {
        subagentTracker.tryClaimChildRun(rid, runRef.id, {
      ...
      // Child runs: progress-only. Never leak tools / lifecycle / items into the parent timeline.
      if (rid && subagentTracker.isKnownChildRun(rid)) {
        if (p.stream === "tool") {
          const d = p.data && typeof p.data === "object" ? /** @type {any} */ (p.data) : {};
          subagentTracker.noteChildToolProgress(rid, {
          ...
          subagentTracker.noteAgentPayload(p);
        }
        const subPayload = subagentTracker.buildSubagentActivityPayload(rid);
        if (subPayload) {
          emit({
            type: "agent_activity",
            stream: "subagent",
        ...
        if (subagentTracker.shouldHoldStreamOpen()) scheduleSubagentFinishCheck();
        else if (subagentTracker.canFinishStream()) tryFinishAfterParentFinal();
        return;
      }
      ...
      if (p.stream === "tool") {
        ...
        const result =
          typeof rawResult === "string"
            ...
            : rawResult && typeof rawResult === "object"
              ? JSON.stringify(jsonSafeDeep(rawResult))
              : undefined;
        const rawPartial = d.partialResult;
        const partialResult =
          typeof rawPartial === "string"
            ? rawPartial
            : rawPartial && typeof rawPartial === "object"
              ? JSON.stringify(jsonSafeDeep(rawPartial))
              : undefined;
        const toolTraceEvt = {
          type: "tool_trace",
        ...
        emit(toolTraceEvt);
        subagentTracker.noteToolTrace(toolTraceEvt);
        emitSubagentSnapshots();
        if (subagentTracker.shouldHoldStreamOpen()) scheduleSubagentFinishCheck();
        return;
      }
      ...
      if (p.stream === "lifecycle") {
        ...
        // Await-spawn / tasks[] turns can surface parent lifecycle.end while chat.final is delayed.
        // Always mark parent final when this turn spawned, then finish or keep polling.
        // (Previously we skipped markParentFinal while children looked active — stream stuck forever.)
        const lifePhase = typeof data.phase === "string" ? data.phase.trim().toLowerCase() : "";
        if (
          (lifePhase === "end" || lifePhase === "complete" || lifePhase === "completed") &&
          (subagentTracker.spawnCalled || subagentTracker.childCount > 0)
        ) {
          subagentTracker.markParentFinal();
          if (subagentTracker.canFinishStream()) {
            tryFinishAfterParentFinal();
          } else {
            scheduleSubagentFinishCheck();
          }
        }
    ...
    const chatRid = typeof p.runId === "string" ? p.runId.trim() : "";
    if (chatRid && !subagentTracker.isKnownChildRun(chatRid)) {
      subagentTracker.tryClaimChildRun(chatRid, runRef.id, {
        sessionKey: typeof p.sessionKey === "string" ? p.sessionKey.trim() : "",
        parentSessionKey: sessionKey,
      });
    }
    // Child chat must never become parent prose / timeline text.
    if (chatRid && subagentTracker.isKnownChildRun(chatRid)) {
      bumpProgress();
      const snap = extractChatPayloadSnapshot(p);
      subagentTracker.noteChildChatProgress(chatRid, snap.text);
      const subPayload = subagentTracker.buildSubagentActivityPayload(chatRid);
      if (subPayload) {
        emit({
          type: "agent_activity",
          stream: "subagent",
      ...
      if (p.state === "final" || p.state === "aborted" || p.state === "error") {
        subagentTracker.noteAgentPayload({
          runId: chatRid,
          stream: "lifecycle",
          data: { phase: p.state === "error" ? "error" : "end" },
        });
        emitSubagentSnapshots();
        if (subagentTracker.canFinishStream()) tryFinishAfterParentFinal();
        else scheduleSubagentFinishCheck();
      }
      return;
    }
    ...
    if (snap.text || snap.thinking) {
      subagentTracker.noteContinuationActivity();
    }
    {
    ...
    if (p.state === "final") {
      ...
      const finalText = preferLongerChatText(lastText, full.text);
      const finalThinking = preferLongerChatText(lastThinking, full.thinking);
      emit({ type: "content_sync", content: finalText, thinking: finalThinking });
      lastText = finalText;
      lastThinking = finalThinking;
  ...
  try {
    ...
    gatewayAcceptedAt = Date.now();
    log.info("[chat.send.perf] gateway_accept", {
      elapsedMs: gatewayAcceptedAt - startedAt,
      prepMs: gatewayAcceptedAt - startedAt,
      runId: runRef.id,
      sessionKey,
      channel,
    });
  ...
  try {
    ...
    if (sessionUsage) {
      emit({
        ...
        authoritative: true,
        usageBreakdown: {
          llmCallCount: sessionUsage.llmCallCount ?? 0,
...
/**
 * Decide how to close a parent chat stream when children are settled.
 * Used to avoid closing on lifecycle.end before chat.final lands.
 * @param {{ chatFinalReceived: boolean; canFinish: boolean; graceArmed?: boolean }} opts
 * @returns {"wait-children" | "finish-now" | "arm-grace" | "wait-grace"}
 */
function decideStreamFinishAction(opts) {
  const chatFinalReceived = Boolean(opts?.chatFinalReceived);
  const canFinish = Boolean(opts?.canFinish);
  const graceArmed = Boolean(opts?.graceArmed);
  if (!canFinish) return "wait-children";
  if (chatFinalReceived) return "finish-now";
  if (graceArmed) return "wait-grace";
  return "arm-grace";
}
