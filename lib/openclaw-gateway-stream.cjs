...
async function dispatchOpenClawGatewayStreamWithResolved(resolved, messages, signal, onEvent, opts = {}) {
  ...
  const subagentTracker = createGatewaySubagentTracker({
    ...
    // Blocking sessions_spawn should settle immediately after child completion;
  ...
  const offEvent = client.onEvent((evt) => {
    if (evt.event === "agent") {
      ...
      if (p.stream === "lifecycle") {
        ...
        // Await-spawn turns can surface parent lifecycle.end while chat.final is delayed/held.
        // Only soft-close when this turn actually spawned subagents — never finish normal turns early.
        ...
        if (
          ...
          if (subagentTracker.shouldHoldStreamOpen()) {
            scheduleSubagentFinishCheck();
          } else if (subagentTracker.canFinishStream()) {
            tryFinishAfterParentFinal();
          } else if (!subagentTracker.hasActiveChildren() && !subagentTracker.yieldPending) {
