...
/**
 * `chat.send` only accepts one user string; the gateway is supposed to keep prior turns in the
 * session transcript, but transcript persistence can be incomplete — embed earlier turns so the
 * model still sees multi-turn context (mirrors the full array built in the renderer).
 * @param {Array<{ role: string; content: string }>} messages
 * @param {unknown} [composerSkill]
 */
function buildChatSendMessage(messages, composerSkill) {
  ...
  let latest = String(turns[lastUserIdx].content ?? "").trim();
  if (!latest) return "";
 ...
 * @param {(evt: { type: string } & Record<string, unknown>) => void} onEvent
 * @param {{ conversationId?: string; composerSkill?: unknown; channel?: "internal" | "wechat"; wechatPeerId?: string; deliver?: boolean }} [opts]
 */
 ...
 * @param {AbortSignal} signal
 * @param {(evt: { type: string } & Record<string, unknown>) => void} onEvent
 * @param {{ conversationId?: string; uiConversationId?: string; composerSkill?: unknown; channel?: "internal" | "wechat"; wechatPeerId?: string; deliver?: boolean }} [opts]
 */
async function dispatchOpenClawGatewayStreamWithResolved(resolved, messages, signal, onEvent, opts = {}) {
  ...
  const { baseUrl, sessionKey: baseSessionKey } = resolved;
  ...
  if (wechatAutoReply) {
    log.info("[chat.send] wechat auto-reply gateway", {
      ...
      sessionKey,
      channel,
      messageChars: buildChatSendMessage(messages, opts.composerSkill).length,
    });
  }
  ...
  const completion = new Promise((resolve, reject) => {
    finishWaiter = (reason) => {
      ...
      finished = true;
      if (reason === "error") {
        reject(new Error(errorEmitted ? "chat_run_error" : "chat_run_error"));
        return;
      }
      resolve();
    };
  ...
  function payloadSessionMatches(p) {
    ...
    if (!p.sessionKey) return true;
    if (p.sessionKey === sessionKey) return true;
    if (conversationId) {
      const safe = sanitizeConversationIdSegment(conversationId);
      const suffix = `#studio:${safe}`;
      if (p.sessionKey === suffix || String(p.sessionKey).endsWith(suffix)) return true;
    }
    return false;
  }
  ...
  const offEvent = client.onEvent((evt) => {
    ...
    const p = /** @type {any} */ (evt.payload);
    if (!p || typeof p !== "object") return;
    if (!payloadSessionMatches(p)) return;
    if (typeof p.runId === "string" && p.runId.trim()) {
      runRef.id = p.runId.trim();
    }
