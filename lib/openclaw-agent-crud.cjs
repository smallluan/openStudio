...
/**
 * @param {string} workspaceDir
 * @param {{ name: string; description: string; avatar: string; soulMd: string; identityMd?: string; agentsMd?: string; userMd?: string; toolsMd?: string; memoryMd?: string }} meta
 */
function writeAgentWorkspaceFiles(workspaceDir, meta) {
  ...
  const userMd = typeof meta.userMd === "string" ? meta.userMd.trim() : "";
  if (userMd) {
    writeFileAtomic(
...
function provisionOpenClawAgent(payload, studioCfg) {
  ...
  writeAgentWorkspaceFiles(workspaceDir, { name, description, avatar, soulMd, identityMd, agentsMd, userMd, toolsMd, memoryMd });
