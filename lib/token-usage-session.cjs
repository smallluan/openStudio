...
function sumAssistantUsageSince(sessionFile, startedAtMs) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (!norm) continue;
    total = sumUsage(total, norm);
    llmCallCount += 1;
    // Same cached tool/system prefix is re-read on each tool-loop LLM call — take max, not sum.
    cacheReadTokens = Math.max(cacheReadTokens, norm.cacheReadTokens ?? 0);
    cacheWriteTokens = Math.max(cacheWriteTokens, norm.cacheWriteTokens ?? 0);
  }
