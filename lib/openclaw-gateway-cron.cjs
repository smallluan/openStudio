...
async function updateCronJob(cfg, jobId, patch) {
  ...
  if (!id) throw new Error("missing_job_id");
  const patchBody =
    patch && typeof patch === "object" && !Array.isArray(patch) && patch.patch && typeof patch.patch === "object"
      ? /** @type {Record<string, unknown>} */ (patch.patch)
      : patch && typeof patch === "object" && !Array.isArray(patch)
        ? patch
        : {};
  return withCronClient(cfg, async (client) => {
    return await client.request("cron.update", { id, patch: patchBody });
  });
}
 ...
 * @param {string} [reason]
 */
async function cancelGatewayTask(cfg, taskId, reason) {
  const id = String(taskId ?? "").trim();
  if (!id) throw new Error("missing_task_id");
  return withCronClient(cfg, async (client) => {
    return await client.request("tasks.cancel", {
      taskId: id,
