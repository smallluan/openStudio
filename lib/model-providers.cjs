...
/**
 * OpenClaw's anthropic-messages transport rejects model calls when catalog entries
 * omit a positive `maxTokens` (e.g. minimax/MiniMax-M2.7).
 */
const GATEWAY_ANTHROPIC_MESSAGES_DEFAULT_MAX_TOKENS = 8192;

/** @param {unknown} api */
function gatewayProviderUsesAnthropicMessages(api) {
  return String(api ?? "").trim() === "anthropic-messages";
}
 ...
 * @returns {boolean}
 */
function ensureGatewayModelRowDefaults(row, providerApi) {
  if (!row || typeof row !== "object" || Array.isArray(row)) return false;
  if (!gatewayProviderUsesAnthropicMessages(providerApi)) return false;
  const maxTokens =
    typeof row.maxTokens === "number" ? row.maxTokens : Number(row.maxTokens);
  if (Number.isFinite(maxTokens) && maxTokens > 0) return false;
  row.maxTokens = GATEWAY_ANTHROPIC_MESSAGES_DEFAULT_MAX_TOKENS;
  return true;
}

/**
 * Ensure every model row under a gateway provider block satisfies transport defaults.
 * @param {{ models?: unknown[]; api?: unknown } & Record<string, unknown>} block
 * @returns {boolean}
 */
function ensureGatewayProviderModelsDefaults(block) {
  if (!block || typeof block !== "object" || Array.isArray(block)) return false;
  const providerApi = typeof block.api === "string" ? block.api : "";
  if (!gatewayProviderUsesAnthropicMessages(providerApi)) return false;
  const rawModels = block.models;
  if (!Array.isArray(rawModels) || rawModels.length === 0) return false;
  let changed = false;
  /** @type {unknown[]} */
  const nextModels = [];
  ...
  if (changed) block.models = nextModels;
  return changed;
}

/** @param {unknown} url */
function isValidHttpBaseUrl(url) {
...
function resolveProviderBaseUrl(provider, userBaseUrl = "", profile) {
  ...
  if (provider === "openai-compatible" || provider === "anthropic-compatible") return custom;
...
function mapNativeProviderToOpenClaw(provider, profile) {
  const id = String(provider || "").trim();
  if (!id || id === "openai-compatible" || id === "anthropic-compatible" || !KNOWN_PROVIDER_IDS.includes(id)) {
    return null;
  }
...
function applyGatewayProviderCatalog(block, catalogMeta) {
  ...
  for (const [key, value] of Object.entries(catalogMeta)) {
    if (key === "baseUrl" && !isValidHttpBaseUrl(value)) continue;
    if (block[key] !== value) {
      block[key] = value;
      changed = true;
    }
