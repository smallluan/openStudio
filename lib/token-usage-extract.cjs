...
function normalizeUsageShape(raw) {
  ...
  const cacheReadVal = cacheRead ?? 0;
  const cacheWriteVal = cacheWrite ?? 0;
  // Provider `input` / `prompt_tokens` is the billed total; cache* fields are metadata (subset/prefix).
  if (inputTokens == null && (cacheReadVal > 0 || cacheWriteVal > 0)) {
    inputTokens = cacheReadVal + cacheWriteVal;
  }
