...
async function commandExistsAsync(bin) {
  ...
  if (process.platform === "win32" && name.toLowerCase() === "python3") {
    // Windows commonly ships `python.exe`; treat it as equivalent for python3-required skills.
    const py = String(process.env.OPEN_STUDIO_BUNDLED_PYTHON || "").trim();
    if (py && fs.existsSync(py)) return true;
  }
  try {
    if (process.platform === "win32") {
      const aliases = name.toLowerCase() === "python3" ? ["python3", "python"] : [name];
      for (const alias of aliases) {
        try {
          await execFileAsync("where", [alias], { timeout: 2500, windowsHide: true });
          return true;
        } catch {
