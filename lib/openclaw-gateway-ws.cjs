...
function looksLikeDevProfile(baseUrl) {
  return /^https?:\/\/(?:127\.0\.0\.1|::1|localhost):19001\b/.test(baseUrl);
}
...
function resolveGateway(cfg) {
  ...
  return { baseUrl, wsUrl: httpToWs(baseUrl) + "/", token, sessionKey };
}
...
async function openGatewayClient(opts, externalSignal) {
  ...
  ws.on("message", (data) => {
    ...
    if (frame.type === "event") {
      ...
      for (const fn of listeners) {
        try {
          ...
          console.error("[openclaw-ws] event listener threw:", err);
        }
      }
      return;
    }
    ...
    if (frame.type === "res" && typeof frame.id === "string") {
      ...
      if (frame.ok === false) {
        entry.reject(new Error("gateway_rpc_error"));
        return;
      }
  ...
  ws.on("close", (code, reasonBuf) => {
    ...
    if (!userClosed) {
      try {
        ...
        console.warn("[openclaw-ws] onConnectionLost threw:", e);
      }
    }
  ...
  ws.on("error", (err) => {
    void err;
    /* `close` follows with the same failure; avoid duplicate user-visible handling */
  });
