...
// ============= signal-exit v3 compatibility =============
// proper-lockfile (and a few legacy deps) do `const onExit = require("signal-exit")`
// and call it as a function. signal-exit v4 only exposes named exports, so the call
// throws "onExit is not a function" when lockfile.js is evaluated.
const originalLoad = Module._load;

/** @param {unknown} exp */
function applySignalExitV3Compat(exp) {
  if (typeof exp === "function") return exp;
  if (!exp || typeof exp !== "object" || typeof exp.onExit !== "function") return exp;
  const fn = exp.onExit;
  fn.onExit = exp.onExit;
  fn.load = exp.load;
  fn.unload = exp.unload;
  fn.signals = exp.signals;
  return fn;
}

/** @param {string} request */
function isSignalExitModuleRequest(request) {
  return /signal-exit[/\\]/i.test(request);
}

/** @param {string} request */
function isSignalExitSpecifier(request) {
  return request === "signal-exit" || /[/\\]signal-exit$/i.test(request);
}

Module._load = function patchedLoad(request, parent, isMain) {
  const exports = originalLoad.call(this, request, parent, isMain);
  if (typeof request === "string" && isSignalExitModuleRequest(request)) {
    const compat = applySignalExitV3Compat(exports);
    if (compat !== exports) {
      const cached = Module._cache[request];
      if (cached) cached.exports = compat;
      return compat;
    }
  }
  return exports;
...
Module.prototype.require = function patchedRequire(request) {
  const exports = originalRequire.call(this, request);
  if (typeof request === "string" && isSignalExitSpecifier(request)) {
    return applySignalExitV3Compat(exports);
  }
  return exports;
};
