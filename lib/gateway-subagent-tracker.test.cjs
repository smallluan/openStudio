...
test("spawn tool phase result settles children and parent final can finish", () => {
  const tracker = createGatewaySubagentTracker({
    parentRunId: "parent-1",
    postCompletionGraceMs: 0,
  });
  ...
  tracker.tryClaimChildRun("child-weather", "parent-1", {
    sessionKey: "agent:main:subagent:weather",
    parentSessionKey: "agent:main:studio:conv",
  });
  tracker.tryClaimChildRun("child-news", "parent-1", {
    sessionKey: "agent:main:subagent:news",
    parentSessionKey: "agent:main:studio:conv",
  });
  // OpenClaw often emits phase:"result" (not "end") when the tool returns.
  tracker.noteToolTrace({
    toolName: "sessions_spawn",
    toolCallId: "tc-result",
    phase: "result",
    ...
    result: JSON.stringify({
      status: "success",
      results: [
        { runId: "child-weather", status: "success", label: "weather-check", result: "28C" },
        { runId: "child-news", status: "ok", label: "news-check", result: "hot" },
      ],
    }),
  });
  assert.equal(tracker.hasActiveChildren(), false);
...
test("parent final force-settles when spawn tool never got a recognized terminal phase", () => {
  const tracker = createGatewaySubagentTracker({
    parentRunId: "parent-1",
    postCompletionGraceMs: 0,
  });
  tracker.noteToolTrace({
    ...
    toolCallId: "tc-stuck",
    phase: "start",
    args: { task: "查天气", taskName: "weather-check" },
  });
  tracker.tryClaimChildRun("child-stuck", "parent-1", {
    sessionKey: "agent:main:subagent:stuck",
    parentSessionKey: "agent:main:studio:conv",
  });
  // Late update without terminal phase/result — previously left openSpawnToolIds + active child.
  tracker.noteToolTrace({
    toolName: "sessions_spawn",
    toolCallId: "tc-stuck",
    phase: "update",
