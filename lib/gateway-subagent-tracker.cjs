...
const TERMINAL_LIFE_PHASES = new Set(["end", "error", "failed", "cancelled", "canceled", "complete", "completed", "ok", "success"]);
// OpenClaw tool frames may use phase "result" / "ok" when the tool returns — treat as terminal.
const TERMINAL_ITEM_PHASES = new Set(["end", "error", "failed", "cancelled", "canceled", "complete", "completed", "result", "ok", "success"]);
const TERMINAL_ITEM_STATUS = new Set(["completed", "complete", "failed", "error", "cancelled", "canceled", "aborted", "ok", "success", "timeout", "forbidden"]);
...
function readSpawnArgs(args) {
  ...
  // Prefer taskName (matches UI toolTraceMerge) so coalesce can attribute per-child done.
  const label =
...
function readSpawnRegistration(obj) {
  ...
  const terminal = ["completed", "complete", "ok", "success", "error", "failed", "timeout", "forbidden"].includes(status);
...
function createGatewaySubagentTracker(opts = {}) {
  ...
  const noteToolTrace = (evt) => {
    ...
    if (isSpawnToolName(toolName)) {
      ...
      if (resultObj) {
        ...
        const resultTerminal = TERMINAL_ITEM_STATUS.has(resultStatus);
        if (batchResults && batchResults.length) {
          ...
          if (toolCallId) {
            settleSpawnToolChildren(toolCallId, "completed");
          ...
          } else if (toolCallId && resultTerminal) {
  ...
  const markParentFinal = () => {
    ...
    // Hard-block / await: parent turn is done. Ghost children or spawn tools that ended with
    // phase "result" (not in older TERMINAL sets) must not keep the stream open forever.
    // Detach+yield keeps yieldPending until barrier/continuation settles.
    if (spawnCalled && !yieldPending) {
      openSpawnToolIds.clear();
      if (hasActiveChildren()) forceSettleAllChildren();
    }
    noteChildrenCompletionClock();
  };
