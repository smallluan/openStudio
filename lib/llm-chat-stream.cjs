...
function resolveDispatch(cfg) {
  ...
  if (provider === "openai-compatible" || provider === "deepseek") {
    ...
    if (!baseUrl) baseUrl = trimTrailingSlash(DEFAULT_BASE_URLS[provider] || "");
    if (!baseUrl) throw new Error("missing_base_url");
  } else {
    baseUrl = trimTrailingSlash(DEFAULT_BASE_URLS[provider] || "");
  }
...
async function dispatchStream(cfg, messages, signal, onEvent) {
  ...
  if (resolved.provider === "openai" || resolved.provider === "openai-compatible" || resolved.provider === "deepseek") {
