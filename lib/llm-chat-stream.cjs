...
function resolveDispatch(cfg) {
  ...
  const needsCustomBaseUrl = provider === "openai-compatible" || provider === "anthropic-compatible";
  if (!baseUrl) throw new Error(needsCustomBaseUrl ? "missing_base_url" : "missing_provider_base_url");
...
async function dispatchStream(cfg, messages, signal, onEvent) {
  ...
  if (resolved.provider === "anthropic" || resolved.provider === "anthropic-compatible") {
