 ...
 *
 * Used by native tools `browser_debug` and `browser_screenshot`.
 */
...
function initPreviewGuestCapture(logger) {
  log = logger ?? null;
  cdp.initPreviewGuestCdp(logger);
...
function attachPreviewGuest(guestContents) {
  ...
  guestContents.on("destroyed", () => {
    ...
    cdp.resetCdpSession();
    try {
      require("./preview-guest-debugger.cjs").resetDebuggerState();
    } catch {
      /* ignore */
    }
  });
...
/** @type {(() => void) | null} */
let unregisterNetworkHandler = null;
 ...
 */
async function onNetworkCdpMessage(method, params, wc) {
  ...
  try {
    if (method === "Network.requestWillBeSent") {
      ...
      if (!requestId) return;
      networkSeq += 1;
      const req = /** @type {Record<string, unknown>} */ (p.request || {});
      const row = {
        id: `req_${networkSeq}`,
        requestId,
        ts: Date.now(),
        ...
        url: String(req.url || ""),
        type: String(p.type || p.resourceType || ""),
        status: null,
        mimeType: "",
        ok: null,
        errorText: "",
        requestHeaders: sanitizeHeaders(/** @type {any} */ (req.headers), false),
        responseHeaders: {},
        body: "",
        bodyTruncated: false,
        guestId: wc.id,
    ...
    if (method === "Network.responseReceived") {
      ...
      if (!row) return;
      const response = /** @type {Record<string, unknown>} */ (p.response || {});
      row.status = Number(response.status) || 0;
      row.mimeType = String(response.mimeType || "");
      row.ok = row.status >= 200 && row.status < 400;
      row.responseHeaders = sanitizeHeaders(/** @type {any} */ (response.headers), false);
      row.type = String(p.type || row.type || "");
      return;
    }
    ...
    if (method === "Network.loadingFailed") {
      ...
      if (!row) return;
      row.ok = false;
      row.errorText = String(p.errorText || p.canceled || "loading_failed");
      return;
    }
...
async function attachDebugger(wc) {
  if (!unregisterNetworkHandler) {
    unregisterNetworkHandler = cdp.registerCdpMessageHandler(onNetworkCdpMessage);
  }
  const attach = await cdp.ensureCdpAttached(wc);
  if (!attach.ok) {
    log?.warn?.("[preview-guest-capture] debugger.attach failed:", attach.message ?? attach.error);
    return false;
  }
  const enabled = await cdp.enableCdpNetwork(wc);
  if (!enabled.ok) {
    log?.warn?.("[preview-guest-capture] Network.enable failed:", enabled.message ?? enabled.error);
...
function buildCatalog(args = {}) {
  ...
  return {
    ...
    logCatalog,
    networkCatalog,
    hint: "User asks what is in the console → call op=console (or fetch with fetchLogs:true). For network first-load use start+reload:true. Do NOT use browser_debugger to read console output.",
  };
}
...
function fetchDetails(args = {}) {
  ...
  if (!logs.length && !network.length) {
    const wantsLogs =
      ...
      levelFilter.length;
    const wantsNetwork =
      args.fetchNetwork === true || networkIds.size > 0 || urlContains || (onlyErrors && !wantsLogs);
    if (wantsLogs && !wantsNetwork) {
      return {
        ok: true,
        recording: getRecordingStatus(),
        logs: [],
        ...
        empty: true,
        hint:
          "No console messages buffered yet for this page. Console is captured continuously — reload or reproduce, then call op=console again. Do NOT use browser_debugger to read console logs.",
      };
    }
    return {
      ok: false,
  ...
  switch (op) {
    ...
    default:
      return {
        ok: false,
        error: "unknown_op",
        message: `Unknown op "${op}". Use start|stop|clear|status|catalog|console|fetch|reload.`,
      };
  }
}
...
async function handleSidebarScreenshot(args = {}) {
  ...
  const result = {
    ...
    url: safeGuestUrl(wc),
    guestId: wc.id,
    hint: "Screenshot saved on disk. Vision-capable models can use this image; otherwise use path as a capture artifact. Prefer DOM inventory + browser_action for interaction.",
  };
  if (includeBase64) {
