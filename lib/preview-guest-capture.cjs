...
function buildCatalog(args = {}) {
  ...
  return {
    ...
    logCatalog,
    networkCatalog,
    hint: "User asks what is in the console → call op=console (or fetch with fetchLogs:true). For network first-load use start+reload:true. Do NOT use sidebar_debugger to read console output.",
  };
}
  ...
  if (!logs.length && !network.length) {
    ...
    if (wantsLogs && !wantsNetwork) {
      return {
        ...
        empty: true,
        hint:
          "No console messages buffered yet for this page. Console is captured continuously — reload or reproduce, then call op=console again. Do NOT use sidebar_debugger to read console logs.",
      };
    }
    return {
      ok: false,
