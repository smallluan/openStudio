...
const { ensureWeixinPluginOpenClawPeerDep } = require("./ensure-openclaw-weixin-plugin.cjs");
const {
...
const WECHAT_MEDIA_SEND_TIMEOUT_MS = 120_000;
...
async function importWeixinPluginModule(accountStateDir, relPath, cfg) {
  ...
  let root = findWeixinPluginRoot(pluginStateDir);
  let peerStateDir = pluginStateDir;
  if (!root) {
    root = findWeixinPluginRoot(accountStateDir);
    peerStateDir = accountStateDir;
  }
  if (!root) throw new Error(WECHAT_PLUGIN_MISSING_ERROR);
  const peer = ensureWeixinPluginOpenClawPeerDep(peerStateDir, { quiet: true });
  if (!peer.ok) {
    throw new Error(String(peer.message ?? "openclaw_peer_dep_missing"));
  }
  const abs = resolveWeixinPluginModulePath(root, relPath);
  if (!abs) throw new Error(WECHAT_PLUGIN_MISSING_ERROR);
 ...
 * @returns {string}
 */
function resolveLocalMediaPath(raw) {
  const mediaPath = String(raw ?? "").trim();
  if (!mediaPath || /^https?:\/\//i.test(mediaPath)) return mediaPath;
  if (mediaPath.startsWith("file://")) {
    let p = new URL(mediaPath).pathname;
    if (process.platform === "win32" && p.startsWith("/")) p = p.slice(1);
    return path.normalize(p);
  }
  if (path.isAbsolute(mediaPath)) return path.normalize(mediaPath);
 ...
 * @param {string} accountStateDir
 * @param {unknown} [cfg]
 * @param {{ peerId: string; mediaPath: string; text?: string; accountId?: string }} args
 */
async function sendWeixinChannelMedia(accountStateDir, cfg, args) {
  const peerId = String(args.peerId ?? "").trim();
  const rawMediaPath = String(args.mediaPath ?? "").trim();
  const text = String(args.text ?? "").trim();
  if (!peerId || !rawMediaPath) throw new Error("wechat_invalid_media_outbound");
  ...
  const baseUrl =
    typeof accountData?.baseUrl === "string" && accountData.baseUrl.trim()
      ...
      : WECHAT_LOGIN_API_BASE;
  const cdnBaseUrl =
    typeof accountData?.cdnBaseUrl === "string" && accountData.cdnBaseUrl.trim()
      ? accountData.cdnBaseUrl.trim()
      : typeof accounts.CDN_BASE_URL === "string" && accounts.CDN_BASE_URL.trim()
        ? accounts.CDN_BASE_URL.trim()
  ...
  /** @type {string} */
  let filePath = resolveLocalMediaPath(rawMediaPath);
  if (/^https?:\/\//i.test(rawMediaPath)) {
    const upload = await importWeixinPluginModule(accountStateDir, "cdn/upload.js", cfg);
    if (typeof upload.downloadRemoteImageToTemp !== "function") {
      throw new Error("wechat_media_download_unavailable");
    }
    const tmpDir = path.join(os.tmpdir(), "open-studio-weixin-outbound");
    fs.mkdirSync(tmpDir, { recursive: true });
    filePath = await upload.downloadRemoteImageToTemp(rawMediaPath, tmpDir);
  } else if (!filePath || !fs.existsSync(filePath)) {
    throw new Error("wechat_media_file_not_found");
  }

  const sendMedia = await importWeixinPluginModule(accountStateDir, "messaging/send-media.js", cfg);
  const result = await sendMedia.sendWeixinMediaFile({
    filePath,
    to: peerId,
    text,
    opts: {
      ...
      token,
      contextToken,
      timeoutMs: WECHAT_MEDIA_SEND_TIMEOUT_MS,
    },
    cdnBaseUrl,
  });
  return {
    ok: true,
    method: `${WECHAT_CHANNEL_ID}.sendMedia`,
    messageId: String(result?.messageId ?? randomUUID()),
    raw: result,
  };
}
...
/**
 * @param {Awaited<import("./openclaw-gateway-ws.cjs").openGatewayClient>} client
 * @param {{ peerId: string; mediaPath: string; text?: string; conversationId?: string; idempotencyKey?: string }} args
 */
async function sendWechatMedia(client, args, cfg) {
  const primaryDir = cfg ? resolveStateDirFromCfg(cfg) : null;
  if (primaryDir && isWeixinChannelPluginAvailable(primaryDir, cfg)) {
    const { stateDir } = resolveWeixinAccountsStateDir(cfg);
    return sendWeixinChannelMedia(stateDir, cfg, args);
  }
  await ensureWechatToolSurface(client, cfg);
  const peerId = String(args.peerId ?? "").trim();
  const mediaPath = String(args.mediaPath ?? "").trim();
  const text = String(args.text ?? "").trim();
  if (!peerId || !mediaPath) throw new Error("wechat_invalid_media_outbound");
  ...
  const { payload, method } = await requestWithFallback(
    client,
    [
      "wechat.outbound.sendMedia",
      "wechat.messages.sendMedia",
      "wechat.message.sendMedia",
      "plugins.wechat.outbound.sendMedia",
      "plugins.wechat.messages.sendMedia",
      "plugins.wechat.message.sendMedia",
    ],
    params,
  );
  const obj = payload && typeof payload === "object" ? /** @type {Record<string, unknown>} */ (payload) : {};
