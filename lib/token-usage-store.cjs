...
function createTokenUsageStore(userDataDir) {
  ...
  let cache = null;
  /** @type {Map<string, Partial<TokenUsageRecord> & { usage?: { inputTokens?: number; outputTokens?: number; totalTokens?: number } }>} */
  ...
  function beginStream(streamId, meta) {
    ...
    pendingByStream.set(sid, { ...prev, ...meta, streamId: sid });
  }
