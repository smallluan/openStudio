 ...
 * @returns {string}
 */
function buildOpenClawCliNodePath() {
  /** @type {string[]} */
  const segments = [];
...
function resolveOpenClawAsarBootstrapPath() {
  const bootstrap = path.join(getProjectRoot(), "scripts", "openclaw-asar-bootstrap.mjs");
  return fs.existsSync(bootstrap) ? bootstrap : null;
}
